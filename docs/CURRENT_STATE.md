...
| 本次 Goal     | `DOC-READY-01：同步数据库实施开工规则`                                                                                                                        |
| 主要证据      | 当前 Git 状态和历史、冻结设计文档、`AGENTS.md`、`docs/DEMO_PLAN.md`、`docs/GOAL_TEMPLATE.md`、当前 Java 源码、测试、SQL 及 DOC-STATE-02 的 Maven 命令结果 |
...
### 4.1 DOC-READY-01 开始时 Git 事实

本次开始时复核结果：

- 当前分支：`feat/database-baseline`；
- Goal 开始时 HEAD：`62893b1`；
- 工作区：干净；
- 当前分支在主线冻结基线 `cef15bc` 之上包含 3 个文档提交；
...
- `CURRENT_STATE`、`DEMO_PLAN` 和 `GOAL_TEMPLATE` 已经完成数据库实施前准备；
- `DB-BASE-01`：准备中；
- 数据库 DDL 实现：尚未开始，本次尚未生成或执行 SQL。
...
| MySQL                   | 运行时驱动已引入，默认配置指向`pharma_erp`            | 配置存在；DOC-READY-01 未验证数据库连接 |
...
以上只是当前仓库开发默认配置。冻结数据库设计要求应用按 UTC 写入并校验会话时区，当前配置尚未完成对应迁移；DOC-READY-01 也没有验证数据源连接、Profile 隔离或文档端点。
...
- 没有空库顺序执行的完整基线；
- 没有仓库内数据库不变量验证脚本；
- 没有证据证明旧 `001_create_drug_table.sql` 已在 `DOC-READY-01` 中执行；
- `DOC-READY-01` 尚未生成或执行 SQL；
- `DOC-READY-01` 未连接、查询或修改任何 MySQL Schema。
...
### 10.1 DOC-STATE-02 复验结果（DOC-READY-01 未重跑）
...
`DOC-STATE-02` 没有复验这些数据库机制，也没有把手工验证结果扩大为 51 张表已经创建或数据库基线已经验收。`DOC-READY-01` 同样未连接数据库或复验这些机制。
...
`docs/GOAL_TEMPLATE.md` v1.0 / 执行模板已经建立，但 `DB-BASE-01` 的正式任务说明尚未形成。该 Goal 仍处于准备中，尚未启动数据库实现，上述 6 张系统权限表尚未创建。本次尚未生成或执行 SQL。

`DP-01`～`DP-03` 仍是实施前决策点，本次未作决定。
