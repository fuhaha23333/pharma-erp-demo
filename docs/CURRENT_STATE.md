...
| 本次 Goal     | `DB-BASE-01规格映射同步复核（文档修订，不启动DDL）`                                                                                                                          |
| 主要证据      | 当前 Git 状态和历史、冻结设计文档、`AGENTS.md`、`docs/DEMO_PLAN.md`、`docs/GOAL_TEMPLATE.md`、`docs/goals/DB-BASE-01.md`、Codex只读复核结果、当前 Java 源码、测试和SQL |
...
- Goal开始时HEAD：`679d1bc`；
- 工作区：干净；
- 当前分支在主线冻结基线`cef15bc`之上包含5个文档和实施规格提交；
...
- `DB-BASE-01`正式执行规格已经形成并提交；
- `DP-01`～`DP-03`已经在执行规格中关闭；
- Codex只读复核发现的角色—权限映射语义问题已在本次文档修订中收口：`QA-016`补充申请人粗粒度关系，关系总数调整为233，直接修改禁令和阶段性禁止边界已明确；
- `DB-BASE-01`仍为准备中，须完成修订提交后的最终只读复核；
- 数据库DDL实现尚未开始，本次尚未生成或执行SQL。

---

## 6. 当前仓库结构

本次规格同步开始时，当前Git跟踪36个文件，其中：
...
| MySQL                   | 运行时驱动已引入，默认配置指向`pharma_erp`            | 配置存在；本次规格同步未验证数据库连接 |
...
以上只是当前仓库开发默认配置。冻结数据库设计要求应用按 UTC 写入并校验会话时区，当前配置尚未完成对应迁移；本次规格同步也没有验证数据源连接、Profile隔离或文档端点。
...
- 没有空库顺序执行的完整基线；
- 没有仓库内数据库不变量验证脚本；
- 没有证据证明旧`001_create_drug_table.sql`已在本次规格同步中执行；
- `DB-BASE-01`执行规格已经形成，但三个实施文件尚未生成；
- 本次规格同步尚未生成或执行SQL，也未连接、查询或修改任何MySQL Schema。
...
### 10.1 DOC-STATE-02复验结果（本次规格同步未重跑）

| 命令                                           | DOC-STATE-02 结果                                           | 能证明什么                         | 不能证明什么                                          |
| ---------------------------------------------- | ----------------------------------------------------------- | ---------------------------------- | ----------------------------------------------------- |
...
`DOC-STATE-02`没有复验这些数据库机制，也没有把手工验证结果扩大为51张表已经创建或数据库基线已经验收。本次规格同步同样未连接数据库或复验这些机制。
...
`docs/goals/DB-BASE-01.md` v1.0 / 执行规格已经形成并提交。`DP-01`～`DP-03`已经在该规格中关闭。

本次根据Codex只读复核结果同步了角色—权限映射：

- `PO-007`、`RC-008`、`SO-012`继续保持直接修改禁令，不生成角色关系；
- `QA-016`增加`PURCHASER`和`SALES_OPERATOR`的本人申请结果粗粒度关系；
- 阶段性禁止与整个动作族绝对禁止已明确分离；
- 角色—权限关系预期总数由231调整为233。

该Goal仍处于准备中，须在本次修订提交后完成最终只读复核。六张系统权限表尚未创建，三个实施文件尚未生成，本次尚未生成或执行SQL。
