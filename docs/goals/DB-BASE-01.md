...
> **文档定位：** 本文件是 `DB-BASE-01` 的正式实施规格，用于关闭六张系统权限表物理字段、角色—权限关系和MySQL物理约束命名的实施阻塞。它不是DDL，不替代原始执行和验收证据，也不修改冻结业务设计。
...
| 执行期Git规则  | 默认不得commit或push；后续经用户明确授权已完成commit和push    |
...
本章描述实现完成后、人工验收和Git提交之前的自检门禁。后续经用户明确授权执行commit和push，不反向破坏验收结论。
...
以上是DB-BASE-01实施前的规格审查结论。
该规格随后已经实施并完成验证，当前完成事实见第21章。
```
...
| --- | --- |
| 实施提交 | `ab8028e` |
| 实施分支 | `feat/database-baseline` |
| 远程同步 | 已push至`origin/feat/database-baseline` |
| 数据库版本 | MySQL 8.4.10 |
| 隔离Schema | `pharma_erp_db_base_01_test` |
| 验证结果 | 通过 |
| 最终Schema状态 | 验证成功后已删除；清理后查询`INFORMATION_SCHEMA.SCHEMATA`的数量为0 |
...
- 六张系统权限表DDL执行成功，共77个字段；
- 8个唯一约束、24个CHECK、10个显式普通索引和21个外键通过验证；
- 10个正式角色、160个权限和233条角色—权限关系通过机械核对；
- 全部规定负向数据库测试通过；
- 负向测试支撑数据清理通过；
- Maven编译结果为`BUILD SUCCESS`；
- Maven测试结果为`Tests run: 1`、`Failures: 0`、`Errors: 0`、`Skipped: 0`。

### 21.2 验证环境与交付边界

- `pharma_erp_db_base_01_test`只用于隔离验收；
- 验证成功后该测试Schema已经删除，最终复查数量为0；
- 本Goal没有把六张表永久部署到默认开发Schema `pharma_erp`；
- 本Goal交付的是可重复执行的数据库基线文件和验证证据。

本Goal仍未实现登录等应用与范围外能力，具体包括：
