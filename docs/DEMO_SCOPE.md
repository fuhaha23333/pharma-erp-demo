...
> **重要说明：** 本文档正在进行业务与合规控制点确认，当前状态为“待冻结”。完成人工确认、测试范围确认并提交 Git 后，方可将状态修改为“冻结基线”。
...
| 追溯管理级别 | 必须，至少支持 `BATCH` 和 `CODE` |
| 是否启用追溯码级管理 | 必须，用于控制码级扫码和校验功能 |
...
> 所有药品经营业务至少必须支持批号级追溯。“是否启用追溯码级管理”只控制码级功能，不表示该药品可以不追溯。
...
5. 销售出库时扫描追溯码；
6. 生成提供给下游的追溯信息；
7. 生成模拟上传请求、成功或失败状态和回执记录；
8. 保留扫码、校验和 Mock 上传审计记录。
...
| 质量管理员 | 首营与资质审核、质量异常确认、冻结和状态处置审批 |
...
| 质量管理员 | 首营审核、客户资质审核、质量事件确认、质量状态处置 | 修改采购订单核心业务数据 |
...
| AC-10 | 质量事件 | 能够创建质量事件、冻结批号库存，并由质量管理员将其处置为合格或不合格，完整保留审计记录。 |
