...
用户于 2026-08-04 明确批准第一阶段以前一阶段已经实现的后端接口作为前端联调契约，并授权同步冻结文档。本次变更只调整第一阶段可演示切片，不表示完整 M1 已经通过：

- 前端采用 Vue 3、Vite、Element Plus，实现登录、当前用户、用户、角色、基础权限、供应商审核和批号追溯页面；
- 第一阶段供应商审核采用 `DRAFT → UNDER_REVIEW → APPROVED / REJECTED`，由具有 `SUPPLIER_REVIEW` 权限且不是提交人的用户作出一次审核决定；
- 第一阶段开放受 `SYS_PERMISSION_WRITE` 保护的基础权限新增和修改接口；
- 第一阶段角色分配由 `SYS_USER_ROLE_ASSIGN` 直接执行，同时校验停用角色和角色互斥；高风险角色申请、审核和批准流程留待后续独立 Goal；
- 第一阶段 `SUPER_ADMIN` 是初始化和演示用高权限角色，具有全部第一阶段权限；不得据此把普通企业管理员解释为万能业务管理员；
- 前端仅提供权限可见性和交互提示，所有授权、状态守卫和职责冲突仍以后端为最终边界；
- HTTP Basic 凭据只保存在前端运行内存中，刷新页面后需要重新登录；部署和演示必须使用 HTTPS。

本变更形成 `PHASE1_CURRENT_API` 实现 Profile。本文其余 M1 完整业务设计继续作为后续目标；与本节列出的第一阶段接口发生差异时，本阶段实现和验收以本节及 `backend/PHASE1_API.md` 为准，但不得据此宣称完整 M1、GSP 合规或生产可用。
...
`PHASE1-FRONTEND-01` 通过只表示当前第一阶段接口具有可操作的浏览器入口，不等于上表全部验收项通过。该 Goal 至少满足：

| 编号 | 验收要求 |
| --- | --- |
| P1-WEB-01 | 正确账号可登录并读取当前用户、角色和权限；错误密码、停用账号和 401 有明确反馈。 |
| P1-WEB-02 | 用户、角色、权限、供应商和追溯入口按真实权限显示；直接调用无权接口仍由后端返回 403。 |
| P1-WEB-03 | 用户启停、角色分配、角色权限配置和权限目录修改均要求填写后端需要的原因或必填字段。 |
| P1-WEB-04 | 供应商可以完成草稿、资质、附件元数据、提交、审核通过或驳回；缺少材料、本人审核和无驳回原因时被阻断。 |
| P1-WEB-05 | 输入生产批号并可选药品编码后，展示真实返回的批次、库存分布和按 UTC 时间排序的事件；无数据和多药品结果不伪造补全。 |
| P1-WEB-06 | 前端自动化测试和生产构建通过，开发代理和生产反向代理边界有说明，仓库不包含依赖目录、构建产物或凭据。 |
