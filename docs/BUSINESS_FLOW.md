...
> **v1.1 第一阶段执行补充（2026-08-04）：** 经用户批准，`PHASE1_CURRENT_API` 采用当前后端已经实现的单次供应商审核流程：采购员维护草稿和资质附件并提交，具有 `SUPPLIER_REVIEW` 权限且不是提交人的审核人直接通过或驳回。原 BF-02 的质量管理员审核、质量负责人最终批准链继续作为完整 M1 后续目标，但不属于本次前端接口契约。本阶段不得据此宣称完整首营审批闭环已经完成。
