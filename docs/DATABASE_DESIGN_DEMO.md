...
| 上位状态机     | `docs/BUSINESS_STATE_MACHINE.md` v1.2                                    |
| 上位权限矩阵   | `docs/ROLE_PERMISSION_MATRIX.md` v1.2                                    |
...
经 2026-08-04 范围变更批准，`PHASE1_CURRENT_API` 复用当前已实施的数据库结构，不在本次前端 Goal 修改数据库：

- 供应商主数据和当前准入状态：`supplier`；
- 供应商资质：`supplier_qualification`；
- 受控附件元数据：`supplier_attachment`；
- 每次提交的不可变审核轮次和结论：`supplier_review`；
- 用户、角色和权限执行：`sys_user`、`sys_role`、`sys_permission`、`sys_user_role`、`sys_role_permission`；
- 直接角色分配的变更证据：`sys_permission_change_log` 和 `sys_operation_log`；
- 业务状态历史：`business_status_history`；
- 批号追溯只读查询：`drug_batch`、`inventory_balance`、`batch_trace_event` 及其关联主数据。

本 Profile 不使用 `supplier_first_approval` 承载当前供应商审核，也不要求前端调用 `sys_role_assignment`。下文完整 M1 候选模型仍用于后续重构目标，不得把尚未由当前迁移和代码使用的候选表写成已实现。

## 0.1 v1.1 状态机与权限矩阵同步摘要
